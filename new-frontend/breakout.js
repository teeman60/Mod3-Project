function breakStart(high_score){
// game parameters
...
KEY_SCORE = "breakout_highscore"; // save key for local storage of high score
...
localStorage.setItem(KEY_SCORE, high_score)
...
var break_ctx = canv.getContext("2d");
...
function drawBackground() {
    break_ctx.fillStyle = COLOR_BACKGROUND;
    break_ctx.fillRect(0, 0, width, height);
}

function drawBall() {
    break_ctx.fillStyle = pupSuper ? PupType.SUPER.color : COLOR_BALL;
    break_ctx.fillRect(ball.x - ball.w * 0.5, ball.y - ball.h * 0.5, ball.w, ball.h);
}
...
function drawBricks() {
    for (let row of bricks) {
        for (let brick of row) {
            ...
            break_ctx.fillStyle = brick.color;
            break_ctx.fillRect(brick.left, brick.top, brick.w, brick.h);
        }
    }
}

function drawPaddle() {
    break_ctx.fillStyle = pupSticky ? PupType.STICKY.color : COLOR_PADDLE;
    break_ctx.fillRect(paddle.x - paddle.w * 0.5, paddle.y - paddle.h * 0.5, paddle.w, paddle.h);
}

function drawPups() {
    break_ctx.lineWidth = wall * 0.35;
    for (let pup of pups) {
        break_ctx.fillStyle = pup.type.color;
        break_ctx.strokeStyle = pup.type.color;
        break_ctx.strokeRect(pup.x - pup.w * 0.5, pup.y - pup.h * 0.5, pup.w, pup.h);
        break_ctx.font = "bold " + pup.h + "px " + TEXT_FONT;
        break_ctx.textAlign = "center";
        break_ctx.fillText(pup.type.symbol, pup.x, pup.y);
    }
}

function drawText() {
    break_ctx.fillStyle = COLOR_TEXT;
    ...
    // labels
    break_ctx.font = labelSize + "px " + TEXT_FONT;
    break_ctx.textAlign = "left";
    break_ctx.fillText(TEXT_SCORE, x1, yLabel, maxWidth1);
    break_ctx.textAlign = "center";
    break_ctx.fillText(TEXT_LIVES, x2, yLabel, maxWidth2);
    break_ctx.fillText(TEXT_LEVEL, x3, yLabel, maxWidth3);
    break_ctx.textAlign = "right";
    break_ctx.fillText(TEXT_SCORE_HIGH, x4, yLabel, maxWidth4);

    // values
    break_ctx.font = textSize + "px " + TEXT_FONT;
    break_ctx.textAlign = "left";
    break_ctx.fillText(score, x1, yValue, maxWidth1);
    break_ctx.textAlign = "center";
    break_ctx.fillText(lives + "/" + GAME_LIVES, x2, yValue, maxWidth2);
    break_ctx.fillText(level, x3, yValue, maxWidth3);
    break_ctx.textAlign = "right";
    break_ctx.fillText(scoreHigh, x4, yValue, maxWidth4);
    ...
    if (gameOver) {
        ...
        break_ctx.font = textSize + "px " + TEXT_FONT;
        break_ctx.textAlign = "center";
        break_ctx.fillText(text, width * 0.5, paddle.y - textSize, maxWidth);
    }
}

function drawWalls() {
    let hwall = wall * 0.5;
    break_ctx.lineWidth = wall;
    break_ctx.strokeStyle = COLOR_WALL;
    break_ctx.beginPath();
    break_ctx.moveTo(hwall, height);
    break_ctx.lineTo(hwall, hwall);
    break_ctx.lineTo(width - hwall, hwall);
    break_ctx.lineTo(width - hwall, height);
    break_ctx.stroke();
}
...
function newScore(score){
    ...
    fetch(`http://localhost:3000/scores/${breakScore.id}`, options)
